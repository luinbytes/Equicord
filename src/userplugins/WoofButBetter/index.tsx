...
// Array of different dog sounds
const dogSounds = ["woof", "wruff", "arf", "bark", "awoo", "ruff", "bow wow", "yip", "howl"];

async function handleButtonClick() {
    // Get a random dog sound from the array
    const randomSound = dogSounds[Math.floor(Math.random() * dogSounds.length)];

    // @ts-expect-error typing issue
    sendMessage(getCurrentChannel().id, { content: randomSound });
}

const ChatBarIcon: ChatBarButtonFactory = ({ isMainChat }) => {
    if (!isMainChat) return null;
    return (
        <ChatBarButton tooltip="Random Dog Sound" onClick={handleButtonClick}>
...
export default definePlugin({
    name: "WoofButBetter",
    description: "Adds a chatbar button to send random dog sounds in chat",
    authors: [Devs.Samwich],
    start: () => addChatBarButton("WoofButBetter", ChatBarIcon),
    stop: () => removeChatBarButton("WoofButBetter")
});
